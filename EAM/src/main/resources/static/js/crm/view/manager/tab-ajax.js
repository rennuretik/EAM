...
function addChildTab() {
	...
	if($('a[href="#no'+key+uniqueCode+'"]').get().length === 0){ // 假如标签不存在
		...
		var tempTab = '<li role="presentation"><a href="#no'+key+uniqueCode+'" role="tab" data-toggle="tab"><span class="tabs-flag">'+$(this).find('p').text()+'</span><span class="close-icon" onclick="removeChildTab(this)">x</span></a></li>';
