...
define ( function ( require ) {
	...
	$ ( function () {
		...
		var leftMenu = new FAPUI.NavgationMenu ( {
			width : 220,
			title:"后台管理",
			url : FAPUI.CONTEXT_PATH + "/menu/getchilds.json?menuCode=mcs.menu",
			listeners : {
				/**
				 * 定义导航菜单单击事件
				 * @param {string} id
				 ...
				 * @param {string} title
				 */
				"itemClick" : function ( id, url, title ) {
		...
		var centerTab = new FAPUI.Layout.TabLayout ( {
			border : false,
			fit : true,
			tabCls : "R",
			items : []
		...
		$(".list_search" ).click(function(){
			...
			$(".search_box" ).show();
			window.location.href = FAPUI.CONTEXT_PATH + "/engine/main?page=lt";
		});
		...
		$(".search_box .ok" ).click(function(){
			window.location.href = FAPUI.CONTEXT_PATH + "/engine/main?page=ht";
		});
		//弹出搜索框定位
		...
		$(".home" ).click(function(){
			window.location.href = FAPUI.CONTEXT_PATH + "/engine/main";
		});
	...
	var modifyPasswordForm = new FAPUI.form.FormPanel({
		...
		onFailure:function(ret){
			Messager.alert("提示信息", undefined === ret.messages ? "系统错误，修改密码失败!" : ret.messages,
	...
	loginUserInfo = function(){
		$.ajax({
			...
			success : function(result){
				userInfoWindow.open();
				userInfoForm.initData(result.result);
