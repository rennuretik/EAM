...
function facultyDelete(id) {
    layer.open({
        ...
        btn: ['确认', '取消'],
        yes: function (index) {
            $.post(contextPath + '/faculty/delete',
                {
                    'tId': id,
                },
                function (model) {
...
function facultyEdit(id) {
    layer.open({
        ...
        btn: ['确认', '取消'],
        yes: function (index) {
            $.post(contextPath + '/faculty/update', $('#form-edit').serialize(), function (model) {
                if (model['result'] == 'success') {
                    alert('编辑系成功');
                    $('#data-query').submit();
                }
...
$(function () {
    ...
    $('#operate-btn1').on('click', function (e) {
        ...
        layer.open({
            ...
            btn: ['确认', '取消'],
            yes: function (index) {
                $.post(contextPath + '/faculty/add', $('#form-edit').serialize(), function (model) {
                    if (model['result'] == 'success') {
                        alert('增加系成功');
                        $('#data-query').submit();
                    }
    ...
    function batchDeleteForm(ids) {
        $.ajax({
            url: contextPath + "/faculty/batch_delete",
            data: {
                "ids": ids,
            },
            type: "post",
