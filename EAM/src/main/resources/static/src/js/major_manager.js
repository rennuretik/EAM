...
function showRightList(treeNode) {
    ...
    $.ajax({
        url: contextPath + "/major/click/list",
        data: {
            "id": treeNode.id,
        },
        type: "post",
        ...
        success: function (data) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                formHtml += "<tr>" +
                    ...
                    "</td>" +
                    "<td id='" + major.mId + "-id' class='id'>" + major.mId + "</td>" +
                    "<td id='" + major.mId + "-mName' >" + major.mName + "</td>" +
                    "<td id='" + major.mId + "-facultyByFId'>" + major.facultyByFId.fName + "</td>" +
                    "<td>" +
                    "<a href='javascript:majorEdit(" + major.mId + ");'>编辑</a>" +
                    "<a href='javascript:majorDelete(" + major.mId + ");'>删除</a>" +
                    "</td>" +
                    "</tr>";
            }
            $("#tbody").html(formHtml);
...
function majorEdit(id) {
    layer.open({
        ...
        btn: ['确认', '取消'],
        yes: function (index) {
            $.post(contextPath + '/major/update', $('#form-edit').serialize(), function (model) {
                if (model['result'] == 'success') {
                    alert('编辑专业成功');
                    if(treeNode1==null){
                        $('#data-query').submit();
...
$('#operate-btn1').on('click', function (e) {
    ...
    layer.open({
        ...
        btn: ['确认', '取消'],
        yes: function (index) {
            $.post(contextPath + '/major/add', $('#form-edit').serialize(), function (model) {
                if (model['result'] == 'success') {
                    alert('增加专业成功');
                    if(treeNode1==null){
                        $('#data-query').submit();
...
function batchDeleteForm(ids) {
    $.ajax({
        url: contextPath + "/major/batch_delete",
        data: {
            "ids": ids,
        },
        type: "post",
