...
function showRightList(treeNode) {
    ...
    $.ajax({
        url: contextPath + "/teacher/click/list",
        data: {
            "id": treeNode.id,
        },
        type: "post",
        ...
        success: function (data) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                formHtml += "<tr>" +
                    ...
                    "</td>" +
                    "<td id='" + teacher.tId + "-id' class='id'>" + teacher.tId + "</td>" +
...
$('#operate-btn1').on('click', function (e) {
    ...
    layer.open({
        ...
        btn: ['确认', '取消'],
        yes: function (index) {
            $.post(contextPath + '/teacher/add', $('#form-edit').serialize(), function (model) {
...
function teacherDelete(id) {
    layer.open({
        ...
        btn: ['确认', '取消'],
        yes: function (index) {
            $.post(contextPath + '/teacher/delete',
                {
                    'tId': id,
                },
                function (model) {
...
function teacherEdit(id) {
    layer.open({
        ...
        btn: ['确认', '取消'],
        yes: function (index) {
            $.post(contextPath + '/teacher/update', $('#form-edit').serialize(), function (model) {
                if (model['result'] == 'success') {
                    alert('编辑教师成功');
                    if(treeNode1==null){
                        $('#data-query').submit();
    ...
    $('#tSex').val($('#' + id + '-tSex').text());
    $('#tAge').val($('#' + id + '-tAge').text());
    $('#tDuty').val($('#' + id + '-tDuty').text());
    $('#isLocal').val($('#' + id + '-isLocal').text());
...
function batchDeleteForm(ids) {
    $.ajax({
        url: contextPath + "/teacher/batch_delete",
        data: {
            "ids": ids,
        },
        type: "post",
